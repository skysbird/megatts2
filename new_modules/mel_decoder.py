...
class MelDecoder(nn.Module):
    def __init__(self, first_channel, num_layers=5, hidden_size=512, kernel_size=5):
        super(MelDecoder, self).__init__()
        self.num_layers = num_layers
        # Assuming the input dimension and output dimension are the same as hidden_size
        layers = [
            nn.ConvTranspose1d(first_channel if l == 0 else hidden_size, hidden_size, kernel_size, stride=1, padding=kernel_size // 2)
